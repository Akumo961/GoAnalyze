...
- `trivy`'s vulnerability database is hosted on a domain outside this sandbox's network allowlist (confirmed via a direct 403, not assumed) -- the binary itself works; dependency scanning was instead done via `pip-audit` and `npm audit` (both real, both clean except one documented, low-risk, unreachable-code-path `ecdsa` finding).
